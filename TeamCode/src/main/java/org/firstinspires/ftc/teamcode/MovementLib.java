...
public class MovementLib {
    ...
    public static class OTOSControl {
        ...
        public void OTOS_Move(double destination_y, double destination_x, double destination_h, double power) {
            SparkFunOTOS.Pose2D pos = this.otos.getPosition();
            double x = pos.x;
            double y = pos.y;
            double h = pos.h;
            if(Math.abs(destination_y - y) > 0.001 || Math.abs(destination_x - x) > 0.001 || Math.abs(destination_h - h) > 0.001) {
                double forward = destination_y - y;
                double strafe = destination_x - x;
                double turn = (destination_h - h) * 1.5;
